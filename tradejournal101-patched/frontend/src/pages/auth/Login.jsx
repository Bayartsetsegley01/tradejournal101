...
export function LoginPage() {
  ...
  const handleSendCode = async (e) => {
    ...
    try {
      await sendCode(inputValue, method);
      setStep('verify');
    } catch (err) {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      await verifyCode(inputValue, code, rememberMe);
      navigate('/app');
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl">
          ...
          {error && (
            <div className="mb-6 bg-rose-500/10 border border-rose-500/20 rounded-lg p-4 text-sm text-rose-400">
              {error}
            </div>
          )}
              ...
              <form onSubmit={handleSendCode} className="space-y-4">
                ...
                <div className="flex items-center">
                  <input
                    ...
                    className="h-4 w-4 bg-slate-950 border-slate-800 rounded text-accent focus:ring-accent"
                  />
