...
export const getTrades = async (req, res) => {
  try {
    ...
    const trades = result.rows.map(t => ({
      ...t,
      entry: t.entry_price, exit: t.exit_price, market: t.market_type,
      date: t.entry_date, rr: t.rr_ratio, stopLoss: t.stop_loss,
      takeProfit: t.take_profit, quantity: t.position_size,
      emotionBefore: t.emotion_before, emotionAfter: t.emotion_after,
      whyEntered: t.why_entered || t.notes, whatHappened: t.what_happened,
      whatWentWell: t.what_went_well, mistakesMade: t.mistakes_made,
      lessonLearned: t.lessons_learned, positiveTags: t.positive_tags || [],
      mistakeTags: t.mistake_tags || [], setupDescription: t.setup_description,
    }));
...
export const addTrade = async (req, res) => {
  try {
    ...
    if (!pnl && entryPrice && exitPrice && positionSize) {
      const ep = parseFloat(entryPrice), xp = parseFloat(exitPrice), qty = parseFloat(positionSize);
      if (!isNaN(ep) && !isNaN(xp) && !isNaN(qty)) pnl = b.direction === 'SHORT' ? (ep-xp)*qty : (xp-ep)*qty;
    }
    let rrRatio = b.rr_ratio || null;
    if (!rrRatio && entryPrice && stopLoss && takeProfit) {
      const ep = parseFloat(entryPrice), sl = parseFloat(stopLoss), tp = parseFloat(takeProfit);
      if (!isNaN(ep) && !isNaN(sl) && !isNaN(tp) && Math.abs(ep-sl) > 0) rrRatio = Math.abs(tp-ep) / Math.abs(ep-sl);
    }
    const result = await query(
      `INSERT INTO trades (user_id, status, symbol, market_type, direction, strategy, session,
        ...
        emotion_before, emotion_after, why_entered, what_happened, what_went_well, mistakes_made,
        positive_tags, mistake_tags, setup_description)
      ...
      [userId, b.status||'CLOSED', b.symbol, marketType, b.direction, b.strategy, b.session,
        ...
        entryPrice, exitPrice, stopLoss, takeProfit, positionSize, pnl, rrRatio, notes, lessonsLearned,
        b.screenshot_url||null, b.emotionBefore||null, b.emotionAfter||null, b.whyEntered||null,
        b.whatHappened||null, b.whatWentWell||null, b.mistakesMade||null,
        JSON.stringify(b.positiveTags||[]), JSON.stringify(b.mistakeTags||[]), b.setupDescription||null]
    );
    const t = result.rows[0];
    res.status(201).json({ success: true, data: {
      ...t, entry: t.entry_price, exit: t.exit_price, market: t.market_type,
      date: t.entry_date, rr: t.rr_ratio, emotionBefore: t.emotion_before,
      emotionAfter: t.emotion_after, whyEntered: t.why_entered||t.notes,
      whatHappened: t.what_happened, lessonLearned: t.lessons_learned,
      positiveTags: t.positive_tags||[], mistakeTags: t.mistake_tags||[]
    }});
  } catch (error) {
...
export const updateTrade = async (req, res) => {
  try {
    if (!getDbStatus()) return res.status(503).json({ success: false, error: 'Database not connected' });
    const { id } = req.params; const userId = req.user.id; const b = req.body;
    const entryPrice = b.entry_price || b.entry || null;
    const exitPrice = b.exit_price || b.exit || null;
    const stopLoss = b.stop_loss || b.stopLoss || null;
    const takeProfit = b.take_profit || b.takeProfit || null;
    ...
    const result = await query(
      `UPDATE trades SET status=$1, symbol=$2, market_type=$3, direction=$4, strategy=$5, session=$6,
        ...
        emotion_before=$19, emotion_after=$20, why_entered=$21, what_happened=$22, what_went_well=$23,
        mistakes_made=$24, positive_tags=$25, mistake_tags=$26, setup_description=$27,
...
export const updateTradeNotes = async (req, res) => {
  try {
    if (!getDbStatus()) return res.status(503).json({ success: false, error: 'Database not connected' });
    const { id } = req.params; const { general_notes, lessons_learned } = req.body;
    const check = await query('SELECT id FROM trade_notes WHERE trade_id = $1', [id]);
    let result;
    if (check.rows.length > 0) result = await query('UPDATE trade_notes SET general_notes=$1, lessons_learned=$2, updated_at=CURRENT_TIMESTAMP WHERE trade_id=$3 RETURNING *', [general_notes, lessons_learned, id]);
    else result = await query('INSERT INTO trade_notes (trade_id, general_notes, lessons_learned) VALUES ($1,$2,$3) RETURNING *', [id, general_notes, lessons_learned]);
    res.json({ success: true, data: result.rows[0] });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
};
...
export const addTradeMedia = async (req, res) => {
  try {
    if (!getDbStatus()) return res.status(503).json({ success: false, error: 'Database not connected' });
    const { id } = req.params; const { file_url, file_type } = req.body;
