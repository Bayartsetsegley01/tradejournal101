...
export function TradeTable({ trades, onRowClick, onEdit, onDuplicate, onDelete, onPatch }) {
  ...
  // Escape cancels edit
  useEffect(() => {
    const h = (e) => { if (e.key === 'Escape') setEditingCell(null); };
  ...
  // Flash + hover overlay for each cell
        ...
        <tbody className="divide-y divide-slate-800/50">
          {trades.map((t) => {
            ...
            return (
              <tr
                key={t.id}
                onClick={() => { if (!editingCell) onRowClick(t); }}
                className={`hover:bg-slate-800/30 transition-all ${editingCell ? 'cursor-default' : 'cursor-pointer'} group`}
              >
                  ...
                  {isEdit(t.id, 'media') ? (
                    ...
                    <>
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {(t.notes || t.whyEntered || t.whatHappened) && (
                          <div className="w-7 h-7 rounded bg-accent/10 border border-accent/20 flex items-center justify-center text-accent" title="Notes байна">
